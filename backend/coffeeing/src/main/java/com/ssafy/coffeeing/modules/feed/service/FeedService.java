...
public class FeedService {
    ...
    @Transactional
    public UploadFeedResponse uploadFeedByMember(Long memberId, String content, List<ImageElement> images) {
        ObjectMapper objectMapper = new ObjectMapper();
        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new BusinessException(MemberErrorInfo.NOT_FOUND));

        try{
            String imageUrl = objectMapper.writeValueAsString(images);
            Feed feed = FeedMapper.supplyFeedEntityBy(member, content, imageUrl);
